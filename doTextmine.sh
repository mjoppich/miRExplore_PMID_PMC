...
if [ "$RUNHGNC" = "TRUE" ]; then
	rm -rf $FOLDER
	mkdir -p $FOLDER
	$SCRIPTPATH/runSyngrep.sh "$TMEXEC --submatch-exclude $SYNBASEFOLDER/excludes/gene_excludes.syn" $FOLDER $SENTENCESPREFIX "" $SYNBASEFOLDER/synonyms/hgnc.syn || exit -1
fi
...
if [ "$RUNMGI" = "TRUE" ]; then
	rm -rf $FOLDER
	mkdir -p $FOLDER
	$SCRIPTPATH/runSyngrep.sh "$TMEXEC --submatch-exclude $SYNBASEFOLDER/excludes/gene_excludes.syn" $FOLDER $SENTENCESPREFIX "" $SYNBASEFOLDER/synonyms/mgi.syn || exit -1
fi
